...
class App : Application(), libtailscale.AppContext {
  ...
  override fun getSyspolicyStringValue(key: String): String {
    ...
    return try {
      AlwaysNeverUserDecidesMDMSetting(key, key)
          .getFrom(manager.applicationRestrictions, this)
          .value ?: ""
    } catch (eanu: IllegalArgumentException) {
      try {
        ShowHideMDMSetting(key, key).getFrom(manager.applicationRestrictions, this).value ?: ""
      } catch (esh: IllegalArgumentException) {
        try {
          StringMDMSetting(key, key).getFrom(manager.applicationRestrictions, this) ?: ""
