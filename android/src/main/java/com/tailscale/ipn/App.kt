...
class App : Application(), libtailscale.AppContext {
  ...
  override fun getSyspolicyStringValue(key: String): String {
    ...
    val alwaysNeverSetting = AlwaysNeverUserDecidesMDMSetting(key, key)
    val showHideSetting = ShowHideMDMSetting(key, key)
    val stringSetting = StringMDMSetting(key, key)

    return when {
        alwaysNeverSetting.keyExists(key) -> alwaysNeverSetting.getFrom(manager.applicationRestrictions, this).value
        showHideSetting.keyExists(key) -> showHideSetting.getFrom(manager.applicationRestrictions, this).value
        stringSetting.keyExists(key) -> stringSetting.getFrom(manager.applicationRestrictions, this) ?: ""
        else -> {
